...
initial_dot, range_print, momentum = None, 2, 0.5
save_img, countour_line_graphic, create_line_connecting_dots = False, True, True
learning_rate, expected_error = 0.1, 0.0001 

equation, symbols = create_function(equation_in_string)
print(symbols)

dot_list, error_list = gradient_descent(equation, symbols, learning_rate=learning_rate, initial_dot=initial_dot, expected_error=expected_error, momentum=momentum)
...
if len(symbols) == 2:
    ...
    if countour_line_graphic:
        generate_countour_line_graphic(dot_list,equation,symbols,equation_in_string,save_img,range_print,create_line_connecting_dots)